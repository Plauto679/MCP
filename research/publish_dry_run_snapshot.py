...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--shadow-report-json",
        default=str(ROOT / "data" / "shadow_paper_trader_live" / "report.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--paper-orders-csv",
        default=str(ROOT / "data" / "shadow_paper_trader_live" / "paper_orders.csv"),
    )
    parser.add_argument(
        "--shadow-actions-csv",
    ...
    *,
    ...
    return {
        ...
        "files": {
            ...
            "paper_orders": str(paper_orders_path),
        },
    }
...
def render_markdown(snapshot: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## PnL",
        "",
        f"- All active PnL: `{all_pnl['active_total_pnl_usd']}` "
        f"(realized `{all_pnl['realized_position_pnl_usd']}`, unrealized `{all_pnl['unrealized_position_pnl_usd']}`)",
        f"- {window_key} active PnL: `{window_pnl['active_total_pnl_usd']}`",
        f"- {fed_key} active PnL: `{fed_pnl['active_total_pnl_usd']}`",
        f"- Open positions: `{all_pnl['open_positions']}`",
        f"- Open quotes: `{all_pnl['open_quotes']}`",
        "",
        "## Reward Proxy",
        "",
...
def compact_history_row(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "all_active_pnl": all_pnl["active_total_pnl_usd"],
        "window_active_pnl": snapshot["pnl"][window_key]["active_total_pnl_usd"],
        "fed_window_active_pnl": snapshot["pnl"][fed_key]["active_total_pnl_usd"],
        "open_positions": all_pnl["open_positions"],
